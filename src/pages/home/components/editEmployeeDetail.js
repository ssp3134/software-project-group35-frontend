...
const EditEmployeeDetail = () => {
    ...
    useEffect(() => {
        const fetchEmployeeData = async (aadhar_card_no) => {
            try {
                if (!aadhar_card_no) {
                    alert("Please fill the Aadhar Card No. field first to update the details.");
                    return;
                }
                const response = await axios.get(`http://localhost:8000/api/employees/${aadhar_card_no}`);
        ...
        fetchEmployeeData(formData.aadhar_card_no);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            try {
                ...
                const response = await axios.put(`http://localhost:8000/api/employees/${formData.aadhar_card_no}`, filteredData, formData.aadhar_card_no);
        ...
        <Grid container spacing={6}>
            <Grid item xs={12}>
                <Card>
                    ...
                    <CardContent>
                    <Typography sx={{ mb: 2 }}>EditEmployeeDetail | Welcomes to Thesis Management System (IIT(ISM)).</Typography>
                        <div>
